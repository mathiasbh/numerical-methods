...
void rkstep45(double t, double h, gsl_vector* y0,
  ...
  for(int i=0; i<n; i++) {
    ...
    gsl_vector_set(yt, i, gsl_vector_get(y0,i)
		   + gsl_vector_get(k1,i)*h*25/216.0
		   + gsl_vector_get(k3,i)*h*1408/2565.0
		   + gsl_vector_get(k4,i)*h*2197/4104.0
		   - gsl_vector_get(k5,i)*h/5.0);
    gsl_vector_set(dy, i, (gsl_vector_get(yt,i) - gsl_vector_get(yh,i)));
  }
  gsl_vector_free(k1);
  gsl_vector_free(k2);
  gsl_vector_free(k3);

...
double adaptClosed(double f(double), double a, double b,
	       ...
	       double nrec, double* err) {
  assert(nrec < 1e7);
  double f2 = f((b+a)/2.0); // midpoint
  double Q  = (f1+4.0*f2+f3)/6.0 * (b-a);
  double q  = (f1+2.0*f2+f3)/4.0 * (b-a);
  ...
  *err = fabs(Q-q);
  if (*err < tol) {
    ...
    double err1, err2;
    double Q1=adaptClosed(f,a,(a+b)/2.0,acc/sqrt(2.0),eps,f1,f2,nrec+1,&err1);
    double Q2=adaptClosed(f,(a+b)/2.0,b,acc/sqrt(2.0),eps,f2,f3,nrec+1,&err2);
    *err = sqrt(err1*err1 + err2*err2);
    return Q1+Q2;
  }
}
...
double intClosed(double f(double), double a, double b,
  ...
  double f3 = f(b);
  int nrec = 0;
  return adaptClosed(f,a,b,acc,eps,f1,f3,nrec,err);
}
...
double clensshawcurtis(double f(double), double a, double b,
		       double acc, double eps, double *err) {
  assert(a == -1 && b == 1);
  double fClen(double t) {return f(cos(t))*sin(t);}
  return intOpen(fClen,0,M_PI,acc,eps,err);
}
